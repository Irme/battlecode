...
public class StaticFunctions {
	...
	public static MapLocation getFirstFreeTile(MapLocation target, RobotController rc){
		Direction[] dirs = Direction.values();
		MapLocation curr = rc.getLocation();
		MapLocation hq = rc.senseHQLocation();
		for(int i = 0; i < 8; i ++){
			MapLocation loc = curr.add(dirs[i]);
			TerrainTile tT = rc.senseTerrainTile(loc);
			if((tT == TerrainTile.NORMAL || tT == TerrainTile.ROAD) && !hq.equals(loc)){
				return loc;
			}
		}
		return null;
